...
namespace HtmlAgilityPack
{
    ...
    public class HtmlWeb
    {
        ...
        //    System.Xml.XmlReader reader = XmlReader.Create(sr);
        ...
        {
            Uri uri = new Uri(url);
            if ((uri.Scheme == UriSchemeHttps) ||
                (uri.Scheme == UriSchemeHttp))
            {
        ...
        public HtmlDocument Load(string url, string proxyHost, int proxyPort, string userId, string password)
        {
            ...
            if ((userId != null) && (password != null))
            {
                ...
                //Add the creds
                credCache.Add(proxyHost, proxyPort, "Basic", myCreds);
                credCache.Add(proxyHost, proxyPort, "Digest", myCreds);
            }
        ...
        /// <param name="writer">The XmlTextWriter to which you want to save.</param>
        //public void LoadHtmlAsXml(string htmlUrl, string xsltUrl, XsltArgumentList xsltArgs, XmlWriter writer)
        //{
        ...
        //            XmlWriter w = XmlWriter.Create(fileStream,new XmlWriterSettings { Encoding= doc.Encoding });
        //            doc.Save(w);
        //            w.Dispose();
        //        }
        //    }
                                   ...
                                   ICredentials creds)
        {
            ...
            if (s != null)
            {
                if (UsingCache)
                {
                    ...
                    SaveStream(s, cachePath, RemoveMilliseconds(DateTime.Now), _streamBufferSize);
        ...
        private string GetCacheHeader(Uri requestUri, string name, string def)
        {
            ...
            using (var fileStream = File.OpenRead(GetCacheHeadersPath(requestUri)))
            {
                doc.Load(fileStream); 
            }
            XmlNode node =
